...
ylabel("\$\\Gamma_{\\mathrm{a}}~(\\mathrm{2\\pi\\cdot Hz})\$")
...
grid()
text(3.8, 0.02,
     ("\$\\Delta\\Gamma_{\\mathrm{m}}=P^2\\cdot2\\pi\\cdot$(fit2_Γ_mol_diff.uncs[1] .* 1000)" *
...
xlabel("Tweezer Power (mW)")
ylabel("\$\\Delta\\Gamma_{\\mathrm{m}}~(\\mathrm{2\\pi\\cdot kHz})\$")
