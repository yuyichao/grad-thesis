...
Originally developed for ion systems to take advantage of
the tight confinement from ion traps~\cite{diedrich_laser_1989,heinzen_quantum-limited_1990},
...
on multiple species~\cite{monroe_resolved-sideband_1995,kerman_beyond_2000,han_3d_2000}.
...
For Cs atoms in our system, the efficient polarization gradient cooling
...
The state of the atom after a Raman $\pi$-pulse is $|1,1\rangle$.
We first use $\sigma^+$ polarized light that is resonant with
the $|1,1\rangle$ to $|2',2\rangle$ transition on the $\mathrm{D2}$ line
to pump the atom out of the $F=1$ manifold to a mixture of $|2,1\rangle$ and $|2,2\rangle$ state.
The beam is generated from the same source as the repumper for the MOT,
which simplifies our setup.

Since the target spin state for the OP, $|2,2\rangle$, is a stretched state,
we can pump the atom from other $F=2$ states into this state
by simultaneously applying $\sigma^+$ light that is resonant with the $F=2$ states.
However, if this is done using a $F=2$ to $F'=3$ transition,
...
Therefore, the OP should be done on a $F=2$ to $F'=2$ transition to minimize this heating.
...
as compared to using OP light resonant with $\mathrm{3^2P_{3/2}}$ to address $F=2$ atoms.
...
Since both the Raman transition and the optical pumping change the hyperfine state of the atom,
