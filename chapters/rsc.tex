...
enables the potential use of Raman sideband cooling~(RSC)
...
achieving a ground state probability of $96(3)~\mathrm{\%}$.
...
The geometry of all the beams and fields involved is shown in Fig.~\ref{fig:rsc:na-geometry}.
...
\item The OP light is turned off and the depth of the trap is lowered so that
...
The energy removed from the atom in one Raman pulse is therefore, $\Delta E_-=\omega$.
...
\footnote{The factor of 4 comes from 2 absorbed photons and 2 re-emitted photons.}
...
that has fewer than $78~\mathrm{\%}$ cooling pulse for $z$~(axial) axis,
...
A cooling sequence can therefore accumulate populations in the dead-zones
...
\footnote{This assumes no coherence between different motional states,
...
varies slightly around the respective expected values
in order to confirm the robustness of the sequence against fluctuation in the experiment.
Fig.~\ref{fig:rsc:na-sequence} demonstrate the resulting optimal sequence from the simulation.
...
Due to the small waist of the Raman beam, it is important to align the Raman beam to
...
The table also includes the total scattering rate from each Raman beam
...
the resonance frequency for these sidebands are not equal spacing anymore due to
