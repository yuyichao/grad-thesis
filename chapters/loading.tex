...
in free space can be found in the previous thesis~\cite{liu_building_2019}.
...
which in turns increases the sensitivity to the alignment
...
and the only method that works for loading tweezers in our experiment
...
due to pairwise loss from light-assisted collision~\cite{schlosser_sub-poissonian_2001}.
...
and the absence of an accessible magic wavelength causes a large ($>\!200~\mathrm{MHz}$)
...
The photon scattered from the atom in the tweezer is collected by the objective
...
The utility of such a system and the possibility it opens up will be discussed
