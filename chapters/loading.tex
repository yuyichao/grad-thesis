...
This cools the atoms to a temperature of $\approx\todo{number}~\mathrm{mK}$~(Na)
and $\approx\todo{number}~\mathrm{mK}$~(Cs)
and is followed by a compressed-MOT~(CMOT) stage
which uses closer to resonant light to push the atoms closer to the center
as well as reducing their temperature to $\approx\todo{number}~\mathrm{mK}$~(Na)
and $\approx\todo{number}~\mathrm{mK}$~(Cs).
...
The MOT is created using six cooling beams of $\approx\todo{number}~\mathrm{mm}$ diameter
