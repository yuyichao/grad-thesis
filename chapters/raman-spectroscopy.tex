...
Moreover, we select $F_{\mathrm{atom}}=6$ as the final total atomic spin state
since this includes the $m_{F_{\mathrm{atom}}}=6$ state which has minimum
...
$m_F=m_{F_{\mathrm{atom}}}+m_{N}=6$ is conserved in the Raman transition,
...
At zero or very low external field,
the good quantum number is $\mathbf{F}=\mathbf{F}_{\mathrm{atom}}+\mathbf{N}$
...
decouples and becomes good quantum numbers individually.
In this basis, the three energy eigenstates are
