...
\item We use the tweezer beam as the Raman beam for transfer in order to reduce scattering
  from external sources\textsuperscript{\ref{ch:raman-spectroscopy:states}}.
...
From section \ref{ch:raman-transfer:raman-model} we see that the transfer efficiency
...
Unlike the selection of final state (see \ref{ch:raman-transfer:state-selction:final})
...
\todo{Refererence interaction shift chapter}
\todo{wavefunction size}
\ref{fig:raman-transfer-atomic-wavefunction}
...
\begin{figure}
  ...
  as intermediate state for the Raman transition]{
    ...
    as intermediate state for the Raman transition.
    The (A) deeply bound excited state ($v'=0$) has a smaller Raman Rabi frequency
    ($\Omega_{Raman}$) compared to the (B) weakly bound excited state ($v'=63$)
    at a given detuning.
    However, the lower scattering rate ($\Gamma_{scatter}$) allows a much larger
    $\Delta_{max}$,
...
States above the Cesium $6^2P_{1/2}$ state,
however, could non-radiatively decay to the Cs $6^2P_{1/2}$ and Na $3^2S_{1/2}$ states
via pre-dissociation which significantly increases the linewidth and should be avoided.

\todo{make sure size mismatch and FCF are mentioned somewhere before this}
The other factor that affects excited state selection is the maximum detuning.
Due to larger FCF with the ground atomic and weakly bound molecular state,
previous attempt at Raman spectroscopy \todo{\cite{}} typically
use an excited state closed to the dissociative threshold as the intermediate state.
...
for any given Raman beam wavelength.
...
\todo{Make sure AOM is defined before and reference/add beam path description,
  mention common mode rejectin by using co-prop beams in beam path}

\section{Raman Transfer Results}

\todo{Make sure beam path/sequence is mentioned previously}
\todo{B field dependency? reference zeeman shift/spin state consideration}
