...
Fortunately, techniques have already been developed to resolve this problem
for experiments that require high spectral purity from the laser.

\todo{
  The solution we use is ...
  Filter out with ASE filter. (spec of filter) show spectrum.
  This is the condition where the data above was taken.
  Compared to the Rabi flopping of ... significantly worse without filter
  corresponding to a ... change in the molecule lifetime.

  Another similar effect comes from the fiber.
  Non-linear effect in fiber can also generate additional frequencies after the fiber.
  Seen in other experiments.
  Although we do not see such noise on the spectrum
  depending on the frequency of the noise it could cause problem even if it's below our threshold.
  We therefore tested adding another filter after the fiber.
  The best result we've got is ... suggesting that the noice
  from the fiber could be a source as well.
  However, since the filter after the fiber potentially have negetive impact on other properties
  of the beam including beam shape, pointing and power stability,
  we were not yet able to make consistent measurement using this.
  Therefore, for the study of the ... below, we use the scattering data from one ASE filter setup.
...
We successfully demostrated the coherent creation of a single molecule in the optical tweezer
using only optical transitions.
...
the beginning of our control on the single molecule
by transfering the control we had on the single atoms.

\todo{
  Molecule created doesn't have strong interaction?

  The natural next step of the experiment is to form a rovibronic ground state molecule
  in order to take advantage of the control in various applications,
  including quantum computing and quantum simulation.
