...
Nevertheless, the transfer efficiency is lower than the prediction from the theory.\todo{
  We believe this is caused by the loss of the molecule,
  show the molecule lifetime data.
...
in Fig.~\todo{ref} is taken.
Comparing it to a similar measurement without the ASE filter shown in Fig.~\todo{ref},
we can see the filter clearly improved the contrast of the oscillation.\todo{
  molecular lifetime also agrees
...
The best result from these tests is shown in Fig.~\todo{ref}
...
\todo{
  The Rabi flopping signal measured above is caused by a combination of many effects,
  making it difficult to identify the cause of the transfer efficiency that's lower than expected.
  Because of this, we vary various experimental parameters
  and measure how they affects the Raman transition.
  This allows us to distinguish between different effect based on their different dependencies.
  For the study of the transition and molecule properties below,
  we measured the scattering rate using the setup with only one ASE filter before the fiber.
  For properties that are not directly related to scattering including light shift
  and Rabi frequency, we will also use data that was taken before the ASE filter was added.

  The parameters that we can easily vary about the transition
  are the power and the detuning of the tweezer\footnote{
    We of course also vary the time and the two photon detuning
    but these used to determine the transition parameters and are not treated as different conditions.
  } and the paramters we can directly measure from the transitions are ... .
  The tweezer power changes the single photon Rabi frequencies both by changing the field strength
  and by changing the confinement which affects the relative atomic wavefunction
  but not the molecular wavefunction.
  We will compare the power scaling measured with the theory calculation
  to check for unknown loss mechanisms.
  Based on the discussion about extra excited states,
  we see that there is a term attributed to
  the desired excited state $|e\rangle$~(in this case the $v'=0$ state)
  ...
  due to the larger detunings from the excited states involved.
  In the study below, we will treat the contribution from other states as constant
  background\footnote{constant here means independent of the detuning
    but could depend on other variables including the tweezer power}
  over the range of frequencies we scan and fit the detuning dependency of each parameters
  as an offset plus the contribution from the $v'=0$ state~(either $1/\Delta$ or $1/\Delta^2$).
  We focus more on the $v'=0$ term since it is easier to compare to the theory
  but we also discuss the effect of the background term on the transfer efficiency.

  We measured the four parameters by fitting the .... to a ... model.
  Additionally, since the atomic loss rate is slow compared to the Raman transition time,
  it is measured separately using the two-body lifetime of the atoms to obtain a more accurate number.
  First we measure the light shift.
  Since $\Omega\ll\Omega$,
}
