...
\chapter{Coherent Optical Creation of a NaCs Molecule}
\label{ch:raman-transfer}

\section{Introduction}
\label{ch:raman-transfer:introduction}

The coherent production of a weakly bound ground-state NaCs molecule is an important milestone
in our two-step approach of creating rovibronic ground-state molecules with full control.
Achieving this goal using only an optical transition without a narrow excited-state linewidth
can allow more laser-coolable atoms to be associated to molecules coherently.
With the characterization of both the excited and ground molecular potentials,
...
and show the results of the coherent molecule creation.
...
and compares it to the Raman transition.
Section~\ref{ch:raman-transfer:state-selection} uses the results from the previous sections
...
$\Gamma_{\mathrm{s}}$, defined as the sum of the scattering rates
out of the initial and final states,
the time of a $\pi$-pulse is $t_{\pi}\equiv\pi/\Omega_{\mathrm{R}}$
and the probability of scattering during this time is,
\begin{align*}
  p_{\mathrm{s}}=&\frac{\Gamma_{\mathrm{s}} t_{\pi}}{2}\\
  =&\frac{\pi\Gamma_{\mathrm{s}}}{2\Omega_{\mathrm{R}}}
\end{align*}
which is proportional to the ratio $\Gamma_{\mathrm{s}}/\Omega_{\mathrm{R}}$.
...
Additionally, other practical limitations in the system such as stabilities of the laser power
...
For the experiment, we use a numerical simulation to find the parameter range
that gives the best transfer efficiency~(section~\ref{ch:raman-transfer:state-selection}).
...
on the transfer efficiency at a qualitative and semiquantitative level.
Due to experimental constraints, we will assume that the single photon detuning is
much smaller than the frequency of each of the individual beams, i.e. $\Delta\ll\nu_1,\ \nu_2$.
...
Next, we will consider the effect of the $|e'\rangle$ states near the excited intermediate state.
...
corresponds to optimal single photon detuning of $0.5\omega'_e$ and $0.15\omega'_e$ respectively.
As one would expect, a larger excited state spacing usually results in
a larger detuning optimum and a better transfer efficiency.

Summarizing the effect of additional excited state as a single number $\Delta_{\max}$ allows
us to keep using the equation for Raman transitions with minor corrections
...
at the same time and the two excited states behave similar to an effective state $|e''\rangle$
...
This is because the coupling between the ground states and the excited states is determined by,
\begin{align*}
...
and $\mathrm{F}_e^i$ and $\mathrm{F}_g$ are the rotation and spin states for the excited
and ground states respectively. Note that the ground states $|i\rangle$ and $|f\rangle$
both have the same $\mathrm{F}_g$. This means
\begin{align*}
  \frac{\Omega_1^{i}}{\Omega_2^{i}}=&\frac{\Omega_1^{0}}{\Omega_2^{0}}
\end{align*}
which is a constant.
Given that the excited-state linewidths for different rotation and hyperfine states
...
The dependence of the ratio on the detuning is not changed significantly
for large detunings.
...
When $\delta\ll\Delta$ such as in our experiment,
\begin{align*}
...
and the ratio between the Raman Rabi frequency and scattering rate is,
\begin{align*}
...
The ratio between the light shift and the Raman Rabi frequency, i.e. line width,
...
i.e. $\Omega_{\mathrm{R}}$, the differential light shift $\Delta\delta$,
and therefore the laser power, must be stabilized to better than $M_1/M_2$.
...
Compared to a Raman transition, which uses detuning from the excited state
...
However, in reality, coupling to states that exist outside the ideal three-level system
...
and discuss which approach should be taken under various circumstances.
...
i.e.,\footnote{This equation and the lower bound on scattering probability applies generally
...
\end{align*}
which is the same as the previous result for a three-level Raman system
where $\Omega_1=\Omega_2$.
In the general case, these give us a lower bound on the scattering probability,
\begin{align*}
  \min\paren{p_{\mathrm{s}0}}=&\frac{4\Gamma_e}{\min\paren{\Omega_1^2, \Omega_2^2}T}
\end{align*}
whereas the real value in a experiement depends on the pulse shape
and may be larger than this lower limit by a small constant factor.
It is also easy to verify that this result agrees with the scattering probability for
an optimal three-level Raman $\pi$-pulse with similar single photon Rabi frequency and pulse time.
This confirms that without additional constraints from the real system or the experimental setup,
neither Raman transfer nor STIRAP offers a significant advantage over the other.
As we will see in the following sections,
other effects in the system can favor one approach over the other.

\subsection{Additional Initial and Final States}
\label{ch:raman-transfer:stirap:extra-init-final}

Similar to the case of
a Raman transition~(section~\ref{ch:raman-transfer:raman:extra-init-final}),
the additional initial and final states cause potential leakage out of the three-level system.
This limits the minimum time of the transfer in a way similar to that of a Raman transition.
...
The excited state will only contribute significantly to the scattering if it causes scattering
...
This allow the time dependent condition above:
$\Omega_1(t)/\Omega_2(t)\neq\Omega'_1(t)/\Omega'_2(t)$
to be expressed as $\Omega_1/\Omega_2\neq\Omega'_1/\Omega'_2$, which is time independent.
...
It is worth noting that although there are not two different cases as in
the discussion for a Raman transition in section~\ref{ch:raman-transfer:raman:extra-ext}
...
As is the case for a Raman transition~(section~\ref{ch:raman-transfer:raman:cross-couple}),
...
a Raman $\pi$-pulse in two limiting cases depending on whether the contribution
...
This is the case where the excited-state separation $\omega_e'$ is significantly smaller compared
to the ground-state separation $\delta$.
By choosing an optimal pulse time, the minimum scattering probability for the STIRAP pulse is,
\begin{align*}
...
Comparing the results and noting that $\Delta_{\max}$ is a fraction of $\omega_e'$,
we see that the two scale similarly to the excited-state spacing.
...
this may not be the case for hyperfine and rotational structures
...
and may be significantly smaller compared to the $\omega_e'$ for the Raman transition,
which is the vibrational state spacing.
As a result, STIRAP may not be as favorable compared to Raman anymore.
The best option in this case would require more in-depth comparisons
...
$\approx\!30~\mathrm{GHz}$, the total scattering probability for a Raman transition
...
\item Apart from scattering, the additional excited and ground state
  also causes a power-dependent, and therefore time-dependent, light shift.
  A STIRAP pulse in the presence of these shifts must vary the frequency of the two beams
  in addition to the power in order to maintain the dark state.
  This is also technically challenging to do.
\end{enumerate}

As a summary, for a two-photon transfer to a weakly bound molecular state,
it is likely that a Raman transition is the preferred technique.
However, for transferring to a deeply bound molecular state,
which is where STIRAP has been used in previous experiments,
STIRAP can have an advantage over a Raman transition
if the energy separation between the initial and final states are large enough
and when the hyperfine and rotational structures in the excited state
does not cause significant scattering.
...
is directly related to the excited-state linewidth, the maximum detuning
and the matrix elements ratio~($M_1/M_2$).
In this section, we will discuss how these, as well as other technical constraints,
affect the choices of states we use for the Raman transfer.
...
\begin{figure}
  ...
  \caption[Enhancement of short range wavefunction]{
    ...
    Compared to other spin states with weaker interactions,
...
shorten the transfer time to decrease sensitivity to technical noise.
...
A strong interaction, either attractive or repulsive,
...
very similar to those of the Cesium D lines between $2\pi\times5~\mathrm{MHz}$ to
$2\pi\times10~\mathrm{MHz}$ due to optical decay processes.
...
for any given Raman beam wavelength using the multi-channel ground-state wavefunctions
provided to us by Jeremy Hutson and taking into account all the states of
...
In addition to the scattering and light shift considerations discussed above,
...
those used in the Raman spectroscopy
...
\item Instead of using a smaller detuning and longer time to maximize the signal of the resonance,
  we increased the single photon detuning to $145~\mathrm{GHz}$
  and used short pulse times~($<\!1~\mathrm{ms}$)
...
\begin{figure}
  ...
  \caption[Raman resonance with Fourier limited linewidth and Rabi flopping]{
    ...
    molecule and then dissociated back to atoms.
...
and the resulting decaying Rabi flop can be seen in
...
without the help of a narrow-linewidth excited state.
...
Below, we will discuss some of the possible causes for the low transfer efficiency.
...
the output path does not have a filter,
...
\begin{figure}
  ...
  \caption[Rabi flopping without ASE filter]{
    Rabi flopping without ASE filter showing a shorter coherence time compared to
    that with one ASE filter. The blue line shows the same data as
    Fig.~\ref{fig:raman-transfer:results:init}B for direct comparison.
    The condition between the two lines are identical otherwise.
    \label{fig:raman-transfer:results:ase:zero}}
\end{figure}

This is a common problem for systems with a large number of excited states,
...
whereas the coherence time of the oscillation is increased from $0.170(10)~\mathrm{ms}$
...
and rely on the specification of the filter
to estimate the cleanliness of the spectrum after the filter.
...
from non-linear effects in the fiber.
...
However, adding the filter after the fiber potentially has a negative impact
on other properties of the tweezer, including beam shape,
power variation and pointing fluctuation,
that we have not been able to characterize yet.
...
we will also use data that were taken before the ASE filter was added.

The parameters of the transition that we can easily vary
...
In other words, we fit the detuning dependence of each property
...
is fitted to $\abs{\psi_{\mathrm{a}}(t)}^2$ from Eq.~\ref{eq:raman-transfer:results:scaling:model}.
...
After repeating this for multiple tweezer powers $P$ and frequencies $f_{\mathrm{twr}}$,
...
and the second term is due to the $v'=0$ state.
Since we have $\Omega_2^2\gg\Omega_1^2$, we assume the $v'=0$ term is only caused by
the coupling between the excited state $|e\rangle$ and the final molecular state $|f\rangle$.
...
For the background term, we have the ``raw'' resonance frequency\footnote{
  Note that the Zeeman shift is still included.} $f_{\mathrm{R}0}$,
...
but the dependence is not significant within the frequency range we measured.
\begin{figure}
  \centering
...
\begin{table}
  ...
  \caption[Fitting result of Raman light shift]{
    ...
    are linear frequencies whereas the quantities used in
    Eq.~\ref{eq:raman-transfer:results:light-shift} and
    \ref{eq:raman-transfer:results:light-shift0} are typically angular frequencies.
...
the near linear dependence of the light shift on the tweezer power.
...
which results in $\approx\!8~\mathrm{\%}$ of the light shift
for the previous measurement~(Fig.\ref{fig:raman-transfer:results:ase:zero})
...
For weak confinements and interactions,
...
For stronger confinements and interactions, however,
...
we use a numerical calculation to determine an approximate scaling of $P^{0.29}$
for our scattering length within the range of tweezer powers we measure.
...
\begin{table}
  ...
  \end{tabular}
\end{table}
We fit the $\Omega_{\mathrm{R}}$ for different detunings to the following model,
\begin{align*}
...
with the $v'=0$ term for a red detuned tweezer which reduces $\Omega_{\mathrm{R}}$
...
makes it difficult to drive a coherent transitions on the blue side.
...
\begin{figure}
  ...
  \caption[Atomic scattering rate as a function of tweezer power and frequency]{
    Dependence of atomic scattering rate $\Gamma_{\mathrm{a}}$
...
the scaling is very close to $P^{2.58}$ instead.
...
\begin{figure}
  ...
  \caption[Molecular scattering rate as a function of tweezer power and frequency]{
    (A) Dependence of molecular scattering rate $\Gamma_{\mathrm{m}}$
    on tweezer power and frequency on a log-log scale.
    The data fits to a linear model $a\cdot P$ better than the atomic scattering rate
    though the higher tweezer power points shows signature of a stronger power dependence.
    The fitting results for tweezer frequencies of
    $288503~\mathrm{GHz}$ and $288560~\mathrm{GHz}$ are
...
which is expected to scale proportionally to the tweezer power.
This agrees with the experimental data better compared to the scattering on the atomic state,
...
The dependence on the tweezer frequency suggests that
...
to study the dependence on detuning and the effect from the $v'=0$ state,
...
is that it couples to an atomic continuum for a different hyperfine state combination.
...
However, this coupling requires changes of $m_F$ quantum number
...
Further verification and potential elimination of this requires
...
using only optical transitions and achieved control over all its degrees of freedoms.
...
We calibrated the parameters of the Raman transition by measuring the dependence
with tweezer power and detuning.
The resulting Rabi frequencies match quite well with the theory prediction
but the scattering rate we observe is much higher than expected.
...
As examples, states with larger binding energies can have weaker coupling
...
the first step towards a fully controlled array of interacting molecules.
As the next step, the molecules will be transferred to the rovibronic ground state
to enable strong dipole interactions, which has recently been achieved
from Feshbach molecule in the optical tweezer
in our experiment~\cite{cairncross_assembly_2021}.
