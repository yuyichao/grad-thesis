...
\item Modeling of the sequence and scan.\\
  ...
  the computer control system must provide a model for each task closed to
...
we will talk about some planned updates to the system in
...
\item Timing
  ...
  \item (Sub-)sequence\\
    ...
    and zero or more subsequences so all the subsequences in a top level sequence form a tree.
  \item Time step\\
...
\item \verb`addStep(<time_params>)` method\\
  ...
  A check makes sure the \verb`curTime` only moves forward and
...
for more info about the hardware specification.
...
\item The sequence supports changing and ramping multiple channels at the same time.
  ...
  A set of active pulses are maintained as the conversion proceeds through sequence time.
...
\item For a continuous ramp of a channel, since the output changes are discrete,
  ...
  to $5~\mathrm{\mu s}$ may only receive updates at time $0, 2, 4~\mathrm{\mu s}$
...
For outputting slowly varying voltage signal, we use an DAQ PCI card
...
to output at each cycle for each channel.
...
USRP supports output of arbitrary spectrum within a certain range.
...
\item Support for more devices can be added to the system.\\
  ...
  These are very similar to the USRP devices but the output has higher bandwidth
