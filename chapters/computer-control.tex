...
and a programmable logic (PL) part, i.e. FPGA.
...
\item Some command corresponds to more than one action/pulse in the sequence.
...
\item The PL command is fixed length to allow a simpler implementation
  of command parsing in the PL.
  However, since not all command contain the same amount of information,
  the command includes padding for alignment.
  In order to save network bandwidth, a compressed format with variable command length
  and no padding is used to transfer the command through the network.
