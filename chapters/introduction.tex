...
relaxing the coherence requirement.

So far, most experiments implemented the first step
by magnetoassociation using a magnetic Feshbach scattering resonance\todo{cite}.
The only exceptions are $\mathrm{Sr}_2$,
where narrow-linewidth ($\sim 20~\mathrm{kHz}$) excited states
are available and optical association can be driven coherently\todo{cite}
and $^{87}\mathrm{Rb}^{85}\mathrm{Rb}$,
where there are molecular states bound by $1\sim2~\mathrm{MHz}$\todo{cite}.
...
in the singlet rovibronic ground state,\todo{figure about dipole moment}
