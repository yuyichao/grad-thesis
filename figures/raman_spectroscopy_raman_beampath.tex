...
\begin{tikzpicture}
  ...
  \end{scope}
  \node[red] at (0, 2.8) {To Experiment};
  \begin{scope}
    \clip (-2, 2.8) rectangle (2, 4);
    \node[blue] at (0, 2.8) {To Experiment};
  \end{scope}
  ...
  % Input PBS
  \drawpbs{(0, 0)}{0.7}{0}
  \node[blue!40!cyan,below right] at (0 + 0.7, 0 - 0.7) {PBS 1};
  % DP AOM
  \drawaom{(-2, 0)}{1}{0.5}{90}
  \node[rotate=-90] at (-2, 0) {\large DP AOM};
  % DP Lens
  \drawlens{(-4.5, 0)}{1}{90}
  \node[blue!80!cyan,below] at (-4.5, -1) {Lens 1};
  \draw[line width=3] (-6 - 0.7, 0 + 0.7) -- (-6 + 0.7, 0 - 0.7);
  % DP QWP
  \drawwaveplate{(-6, 1.5)}{1}{0}
  \node[blue!80!black,right] at (-5.1, 1.5) {$\lambda/4$};
  \draw[line width=3] (-6 - 0.7, 2.5) -- (-6 - 0.1, 2.5 + 0.1);
  % Post DP QWP
  \drawwaveplate{(-6, 3.5)}{1}{0}
  \node[blue!80!black,right] at (-5.1, 3.5) {$\lambda/4$};
  \draw[line width=3] (-6 - 0.7, 5 + 0.7) -- (-6 + 0.7, 5 - 0.7);
  % Post DP Lens
  \drawlens{(-7.5, 5)}{1}{90}
  \node[blue!80!cyan,above] at (-7.5, 6) {Lens 2};
  % SP PBS
  \drawpbs{(-9, 5)}{0.7}{0}
  \node[blue!40!cyan,above] at (-9, 5 + 0.7) {PBS 2};
  ...
  \node[blue!80!black,left] at (-9.9, 1.7) {$\lambda/2$};
\end{tikzpicture}
