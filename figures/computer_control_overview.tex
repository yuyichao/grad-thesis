...
\begin{tikzpicture}
  ...
  \node[draw,text width=1.4cm,align=center] at (-3.3, -4) (FPGA Device) {FPGA\\Server};
  \node[draw,text width=1.4cm,align=center] at (-1.5, -4) (NIDAQ Device) {NiDAQ\\Card};
  \node[draw,text width=1.4cm,align=center] at (0.3, -4) (USRP Device) {USRP\\Server};
  \node at (1.8, -4) {$\cdots$};
  \node[draw,text width=1.4cm,align=center] at (3.3, -4) (X Device) {Other\\Hardware};
  ...
  at (0, -5.7) (Output) {Physical Outputs};
  ...
  node[rotate=90,below,green!40!black] at (4.3, {(\y1 + \y2) / 2}) {\scriptsize Software};
